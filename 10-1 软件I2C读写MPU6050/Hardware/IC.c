...
void PWMI_Init(void)
{
    ...
    TIM_ICInitStruct.TIM_ICPrescaler = TIM_ICPSC_DIV1;
    TIM_ICInitStruct.TIM_ICSelection = TIM_ICSelection_DirectTI;
	TIM_PWMIConfig(TIM3, &TIM_ICInitStruct);//此函数会自动配置此通道的另一个TIM_ICSelection为相反配置
...
{
    return 100000 / (TIM_GetCapture1(TIM3) + 1);//CNT的最后一次计数没加上，遂手动加1
}

uint32_t IC_GetDuty(void)
{
    return 100 * (TIM_GetCapture2(TIM3) + 1) / (TIM_GetCapture1(TIM3) + 1);//CNT的最后一次计数没加上，遂手动加1
}
