...
void MPU6050S_Init(void)
{
    ...
    MPU6050S_WriteReg(MPU6050_PWR_MGMT_1,0x01);
    MPU6050S_WriteReg(MPU6050_PWR_MGMT_2,0x00);
    MPU6050S_WriteReg(MPU6050_SMPLRT_DIV,0x09);
    MPU6050S_WriteReg(MPU6050_CONFIG,0x06);
    MPU6050S_WriteReg(MPU6050_GYRO_CONFIG,0x18);
    MPU6050S_WriteReg(MPU6050_ACCEL_CONFIG,0x18);
...
void MPU6050S_WriteReg(uint8_t Addr, uint8_t Data)
{
    MyI2C_Start();
...
void MPU6050S_WriteRegs(uint8_t Addr, uint8_t *Array, uint8_t Size)
{
    MyI2C_Start();
    MyI2C_SendByte(MPU6050_ADDRESS_LOW | WRITE);
    Assert_ReceiveAck();
...
uint8_t MPU6050S_ReadReg(uint8_t Addr)
{
    MyI2C_Start();
...
void MPU6050S_ReadRegs(uint8_t Addr, uint8_t *ParamArray, uint8_t Size)
{
    MyI2C_Start();
...
void MPU6050S_GetData(int16_t ParamArray[6])
{
    MPU6050S_ReadRegs(MPU6050_ACCEL_XOUT_H, (uint8_t *)ParamArray ,6);
    
    MPU6050S_ReadRegs(MPU6050_GYRO_XOUT_H, (uint8_t *)&ParamArray[3] ,6);
